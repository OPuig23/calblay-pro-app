...
// URLs dels CSV de personal i incidències
...
// Base URL de l'API de notificacions, configurada via variable d'entorn
...
export default function DetailView({ event, onBack, token, setToken }) {
  ...
  // Carrega dades de personal i incidències quan es canvia de tab
  useEffect(() => {
  ...
  const handleNotifyResponsables = () => {
    fetch(`${API_BASE}/api/avis-responsable`, {
      method: 'POST',
      ...
      .then(res =>
        res.ok
          ? alert('✅ Correu enviat al responsable!')
      ...
      <div className="min-h-screen bg-green-50 p-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6">
          ...
          <div className="mt-2 text-sm flex items-center space-x-4">
            ...
            {event.location && (
              ...
              >
                <MapPin className="mr-1" />{event.location}
              </a>
            )}
          </div>
        </div>
        <button onClick={() => setIsAddingIncident(true)} className="w-full py-3 mb-4 bg-orange-500 text-white font-semibold rounded-xl shadow">
          + Incidència
        </button>
        <button onClick={handleNotifyResponsables} className="w-full py-3 mb-4 bg-green-700 text-white font-semibold rounded-xl shadow">
        ...
        {tabsDef.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className="w-full py-3 mb-3 rounded-xl text-white font-semibold shadow"
  ...
  return (
    <div className="min-h-screen bg-green-50 p-4">
      <button onClick={() => setTab(null)} className="text-blue-600 mb-4 flex items-center">
        <ArrowLeft className="mr-1" />Menú
      </button>
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        <h2 className="text-2xl font-bold">{event.name}</h2>
        <div className="mt-2 text-sm flex items-center space-x-4">
